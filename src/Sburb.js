...
var Sburb = (function (Sburb) {
    ...
    Sburb.testCompatibility = function (div, levelName, includeDevTools) {
        ...
        if (!Modernizr.fontface)
            errors.push("- Lack of CSS @font-face support.");
        if (!Modernizr.canvas) errors.push("- Lack of canvas support.");
        if (!Modernizr.canvastext)
            errors.push("- Lack of canvas text support.");
        ...
        if (Sburb.updateLoop && !Sburb.inputDisabled) {
            ...
            if (Sburb.chooser.choosing) {
                ...
                if (
                    ...
                    Sburb.performAction(
                        Sburb.chooser.choices[Sburb.chooser.choice]
                    );
    ...
    function focusCamera() {
        ...
        if (!Sburb.destFocus) {
            ...
            Math.abs(Sburb.destFocus.x - Sburb.cam.x - Sburb.Stage.width / 2) >
                4 ||
            Math.abs(Sburb.destFocus.y - Sburb.cam.y - Sburb.Stage.height / 2) >
                4
        ) {
            Sburb.cam.x +=
        ...
        Sburb.Stage.x = Math.max(
            0,
            Math.min(
                Math.round(Sburb.cam.x / Sburb.Stage.scaleX) *
                    Sburb.Stage.scaleX,
        ...
        Sburb.Stage.y = Math.max(
            0,
            Math.min(
                Math.round(Sburb.cam.y / Sburb.Stage.scaleX) *
                    Sburb.Stage.scaleX,
    ...
    function chainActionInQueue(queue) {
        if (queue.curAction.times <= 0) {
            if (queue.curAction.followUp) {
                if (
                    hasControl() ||
                    queue.curAction.followUp.noWait ||
                    queue.noWait
                ) {
