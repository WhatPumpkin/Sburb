var Sburb = (function (Sburb) {
    ...
    Sburb.Dialoger.prototype.nextDialog = function () {
        ...
        if (prefix == "!") {
            ...
            if (this.actor == null) {
                ...
                var desiredPos = this.startOnSide(
                    this.oppositeSide(this.dialogSide)
                );
    ...
    Sburb.Dialoger.prototype.moveToward = function (sprite, pos, speed) {
        ...
        if (Math.abs(sprite.x - pos.x) > speed) {
            sprite.x +=
                (speed * Math.abs(pos.x - sprite.x)) / (pos.x - sprite.x);
        } else {
            sprite.x = pos.x;
        }
        ...
        if (Math.abs(sprite.y - pos.y) > speed) {
            sprite.y +=
                (speed * Math.abs(pos.y - sprite.y)) / (pos.y - sprite.y);
        } else {
            sprite.y = pos.y;
        }
    ...
    Sburb.Dialoger.prototype.update = function () {
        ...
        if (this.talking) {
            ...
            if (this.actor != null) {
                if (
                    ...
                    sprite1.x = pos1.x;
                    sprite1.y = pos1.y;
                    var pos2 = this.startOnSide(
                        this.oppositeSide(this.dialogSide)
                    );
                    var sprite2 = this.dialogOnSide(
                        this.oppositeSide(this.dialogSide)
                    );
                    sprite2.x = pos2.x;
                    sprite2.y = pos2.y;
        ...
        if (this.type == "social") {
            ...
            if (this.dialogSide == "Right") {
                ...
                spadeButton.x =
                    hashTagBar.x + hashTagBar.animation.colSize - 120;
                heartButton.x =
                    hashTagBar.x + hashTagBar.animation.colSize - 80;
                bubbleButton.x =
                    hashTagBar.x + hashTagBar.animation.colSize - 40;
            }
            ...
            if (this.actor) {
                if (Sburb.buttons.spadeButton.animation.name == "state1") {
                    ...
                    Sburb.buttons.heartButton.animation.name == "state1"
                ) {
                    this.choices[this.currentDialog] = 1;
                } else {
                    this.choices[this.currentDialog] = 0;
    ...
    Sburb.parseDialoger = function (dialoger) {
        ...
        var hiddenPos = parseDimensions(
            attributes.getNamedItem("hiddenPos").value
        );
        var alertPos = parseDimensions(
            attributes.getNamedItem("alertPos").value
        );
