var Sburb = (function (Sburb) {
    ...
    function Sprite(name, x, y, width, height, dx, dy, depthing, collidable) {
        ...
        this.height = height;
        this.depthing =
            typeof depthing == "number" ? depthing : this.BG_DEPTHING; //bg, fg, or mg
    ...
    Sprite.prototype.isVisuallyUnder = function (x, y) {
        return (
            this.animation &&
            this.animation.isVisuallyUnder(x - this.x, y - this.y)
        );
    };
    ...
    Sprite.prototype.getActions = function (sprite) {
        ...
        for (var i = 0; i < this.actions.length; i++) {
            ...
            if (
                ...
                desired == sprite.name ||
                (desired.charAt(0) == "!" &&
                    desired.substring(1) != sprite.name)
            ) {
    ...
    Sburb.parseSprite = function (spriteNode, assetFolder) {
        ...
        var temp;
        newName = (temp = attributes.getNamedItem("name"))
            ? temp.value
            : newName;
        newX = (temp = attributes.getNamedItem("x"))
            ? parseInt(temp.value)
            : newX;
        newY = (temp = attributes.getNamedItem("y"))
            ? parseInt(temp.value)
            : newY;
