...
*/
    ...
    Sburb.FontEngine.prototype.setStyle = function (
        ...
        this.charWidth =
            typeof charWidth == "number" ? charWidth : this.charWidth;
        this.parseText();
    };
    ...
    Sburb.FontEngine.prototype.parseText = function () {
        ...
        for (i = 0; i < this.text.length; i++) {
            ...
            if (
                Sburb.stage.measureText(this.text.substring(lineStart, i + 1))
                    .width > this.width
            ) {
    ...
    Sburb.FontEngine.prototype.nextBatch = function () {
        ...
        this.lines.splice(
            ...
            Math.min(
                this.lines.length,
                Math.floor(this.height / this.lineHeight)
            )
        );
    ...
    Sburb.FontEngine.prototype.draw = function () {
        ...
        while (
            ...
            if (
                currentFormat < this.formatQueue.length &&
                this.formatQueue[currentFormat].minIndex <
                    lenCount + curLine.length
            ) {
            ...
            for (var k = 0; k < currentFormats.length; k++) {
                if (currentFormats[k].maxIndex < this.end) {
                    nextStop = Math.min(
                        nextStop,
                        currentFormats[k].maxIndex - lenCount
                    );
                }
            }
