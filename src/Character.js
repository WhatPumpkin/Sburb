var Sburb = (function (Sburb) {
    ...
    Sburb.Character.prototype.handleInputs = function (pressed, order) {
        ...
        up = Math.max(
            order.indexOf(Sburb.Keys.up),
            order.indexOf(Sburb.Keys.w)
        );
        left = Math.max(
    ...
    Sburb.Character.prototype.serialize = function (output) {
        ...
        for (var animation in this.animations) {
            ...
            if (
                this.bootstrap ||
                (anim.name.indexOf("idle") == -1 &&
                    anim.name.indexOf("walk") == -1)
            ) {
    ...
    Sburb.parseCharacter = function (charNode, assetFolder) {
        ...
        var newChar = new Sburb.Character(
            ...
            attributes.getNamedItem("idleAnimLength")
                ? parseInt(attributes.getNamedItem("idleAnimLength").value)
                : 1,
            attributes.getNamedItem("walkAnimLength")
                ? parseInt(attributes.getNamedItem("walkAnimLength").value)
                : 2,
